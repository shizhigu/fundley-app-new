-- Migration: Insert default watchlist template for all users
-- Description: Create "Default Analysis (ROCE + Options)" template for each existing user
-- Date: 2025-10-25

-- Insert default template for all users who don't already have it
INSERT INTO sql_templates (user_id, name, sql_query, description)
SELECT
    u.id AS user_id,
    'Default Analysis (ROCE + Options)' AS name,
    $TEMPLATE$WITH watchlist_symbols AS (
    SELECT * FROM (VALUES
        {{WATCHLIST_SYMBOLS}}
    ) AS t(symbol)
),
...
ORDER BY w.symbol;$TEMPLATE$ AS sql_query,
    'Comprehensive analysis of your watchlist stocks including ROCE (Return on Capital Employed), price performance vs 26-week high, and optimal put option premiums for income generation.' AS description
FROM users u
WHERE NOT EXISTS (
    SELECT 1
    FROM sql_templates st
    WHERE st.user_id = u.id
    AND st.name = 'Default Analysis (ROCE + Options)'
);

-- Add comment
COMMENT ON TABLE sql_templates IS 'SQL templates for watchlist analysis. Default template includes ROCE and options analysis.';
