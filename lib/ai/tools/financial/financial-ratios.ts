...
export const getFinancialRatios = tool({
  description: `Get financial ratios data for companies. 
  REQUIRED PARAMETERS:
  ...
  inputSchema: z.object({
    ...
    fields: z.array(z.string()).describe('REQUIRED: Specific field names from searchFinancialFields results (e.g., ["grossProfitMargin", "priceToEarningsRatio", "currentRatio"]). NEVER leave this empty.'),
    ...
    try {
      ...
      const allResults = await Promise.all(
        symbols.map(async (ticker) => {
          try {
            const data = await fmpClient.getFinancialRatios(
              ticker,
              period,
              limit
            )
      ...
      if (symbols.length === 1 && successful.length === 1) {
        ...
        if (!data || data.length === 0) {
          return {
            success: false,
            message: `No financial ratios data found for ${ticker}`
          }
        }
        ...
        return {
          ...
          instruction: `Here is the financial ratios data for ${ticker}. Please analyze based on the user's question.`,
          displayAction: `Fetching ${ticker} financial ratios`,
          displayResult: `Retrieved financial ratios for ${ticker.toUpperCase()}`  // 简短信息给前端
        }
      }
      ...
      return {
        ...
        instruction: `Here is the financial ratios data for ${symbolsDisplay}. Please analyze and compare based on the user's question.`,
        // User-friendly display fields
        displayAction: `Fetching financial ratios for ${symbolsDisplay}`,
      ...
      return {
        success: false,
        message: `Failed to fetch financial ratios: ${error instanceof Error ? error.message : 'Unknown error'}`
      }
    }
  }
})

// Tool for discovering what ratio fields to use
export const findFinancialRatioFields = tool({
  description: 'Search for financial ratio field names based on user query. Use this before getFinancialRatios to identify the correct field names.',
  inputSchema: z.object({
    query: z.string().describe('What the user is looking for (e.g., "profit margin", "PE ratio", "liquidity", "杠杆率")')
  }),
  execute: async ({ query }) => {
    ...
    return {
      ...
      // User-friendly display fields
      displayAction: 'Searching financial ratio fields',
